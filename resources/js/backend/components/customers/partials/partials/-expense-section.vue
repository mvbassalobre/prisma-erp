                                ...
                                <table class="table table-striped table-sm mb-0">
                                    ...
                                    <tbody>
                                        <tr v-for="(q, y) in section.expenses" :key="q.id">
                                            <td>
                                                <edit-input @change="changeValue(q, m)" v-model="q.name" />
                                            </td>
                                            <template v-for="(m, i) in months">
                                                <td :key="`${i}_${y}_body`">
                                                    <edit-input type="number" v-model="q[m.value]" :currency="true" @change="changeValue(q, m)" />
                                                </td>
                                            </template>
                                            <td class="text-center">
                                                <button v-loading="loading_expenses" class="append-btn" type="button" @click="deleteExpense(q)">
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <button v-loading="loading_expenses" class="append-btn" type="button" @click.prevent="addExpense">
...
<script>
export default {
    props: ['section', 'years', 'year', 'customer', 'customer_area'],
    data() {
        return {
    ...
    methods: {
        ...
        loadExpenses() {
            ...
            this.$http
                .post('/admin/api/get-data/sectionExpenses', this.section)
                .then((resp) => {
                    resp = resp.data
        ...
        change() {
            this.loading_expenses = true
            this.$http
                .put(`/admin/customers/${this.customer.code}/attendance/edit-section`, this.section)
                .then((resp) => {
                    resp = resp.data
                    resp.sections.map((section) => {
                        let y = this.years.find((year) => year.id == section.year_id)
                        this.$parent.setYearSection(y.value)
                    })
                    this.loading_expenses = false
                })
                .catch((er) => {
        ...
        changeValue(expense, current_month = null) {
            ...
            this.loading_expenses = true
            this.$http
                .put(`/admin/customers/${this.customer.code}/attendance/edit-expense`, expense)
                .then((resp) => {
                    resp = resp.data
                    resp.sections.map((section) => {
                        let y = this.years.find((year) => year.id == section.year_id)
        ...
        deleteSection() {
            this.$confirm('Deseja excluir ?', 'Confirmação', {
                ...
                this.loading_expenses = true
                this.$http
                    .delete(`/admin/customers/attendance/delete-section/${this.section.id}`)
                    .then((resp) => {
                        resp = resp.data
        ...
        addExpense(section) {
            ...
            this.$http
                .post(`/admin/customers/${this.customer.code}/attendance/add-expense`, { ...this.form, section_id: this.section.id })
                .then((resp) => {
                    resp = resp.data
                    resp.sections.map((section) => {
                        let y = this.years.find((year) => year.id == section.year_id)
        ...
        deleteExpense(expense) {
            this.$confirm('Deseja excluir ?', 'Confirmação', {
                ...
                this.$http
                    .delete(`/admin/customers/attendance/delete-expense/${expense.id}`)
                    .then((resp) => {
                        resp = resp.data
                        this.$parent.sections = resp.sections
                        this.refreshForm()
