<template>
    <div class="mt-4" v-loading="loading_expenses">
        ...
        <cash-flow-months :year="year" :sections="sections" />

        <cash-flow :customer="customer" :year="year" :sections="sections" />
    </div>
</template>
<script>
export default {
    props: ['years', 'year', 'customer', 'customer_area'],
    data() {
        return {
            attempts: 0,
            sections: [],
    ...
    methods: {
        ...
        setYearSection(year) {
            let comp = this.$parent.$parent.$parent.$refs[`expense_${year}`]
            if (!comp) return
            comp[0].getSections()
        },
        getSections() {
            ...
            this.$http
                .post('/admin/api/get-data/customerYearSections', this.year)
                .then((resp) => {
                    resp = resp.data
        ...
        addSection() {
            this.$prompt('Digite o nome da sessão de deseja criar', 'Adicionar Sessão', {
                ...
                let section = value
                this.$http
                    .post(`/admin/customers/${this.customer.code}/attendance/add-sections`, { section, year: this.year })
                    .then((resp) => {
                        resp = resp.data
                        this.loading_expenses = false
                        resp.sections.map((section) => {
                            let y = this.years.find((year) => year.id == section.year_id)
                            this.setYearSection(y.value)
                        })
                    })
                    .catch((er) => {
                        console.log(er)
