        ...
        <div class="collapse navbar-collapse d-flex align-items-center">
            ...
            <ul class="navbar-nav d-none d-none d-lg-block ml-auto">
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle d-flex align-items-center flex-row" href="#"
                        id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <b class="mr-2">{{user.email}}</b>
                        <img v-if="user.avatar" class="img-profile" :src="user.avatar" />
                        <span v-else
                            class="text img-profile d-flex align-items-center text-center justify-content-center color">
                            {{user.name.slice(0,2).toUpperCase()}}
                        </span>
                    </a>
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdownMenuLink">
                        <a class="dropdown-item" :href="routeprofile"><span
                                class="el-icon-user mr-2 font-weight-bolder"></span>Minha Conta</a>
                        <a class="dropdown-item" v-if="['admin'].includes(user.roleName)" :href="routesettings"><span
                                class="el-icon-s-tools mr-2 font-weight-bolder"></span>Parâmetros</a>
                        <a class="dropdown-item" :href="routelogout"><span
                                class="el-icon-close mr-2 font-weight-bolder"></span>Logout</a>
                    </div>
                </li>
            </ul>
        </div>
...
<script>
export default {
    props: ["user", "routeprofile", "routelogout", "routesettings"],
    methods: {
        setCollapse() {
